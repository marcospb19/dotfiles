...
parse_git_where() {
	...
	if [ "$NUM_AHEAD" -gt 0 ]; then
		NUM_AB_STATE=$NUM_AB_STATE${GIT_PROMPT_AHEAD}${NUM_AHEAD}
	fi
	local NUM_BEHIND="$(git log --oneline ..@{u} 2> /dev/null | wc -l | tr -d ' ')"
	if [ "$NUM_BEHIND" -gt 0 ]; then
		NUM_AB_STATE=$NUM_AB_STATE${GIT_PROMPT_BEHIND}${NUM_BEHIND}
	fi
	...
	if [ $NUM_AB_STATE ]; then
		NUM_AB_STATE="%{$reset_color%}|$NUM_AB_STATE"
	fi

	echo "$branch_name$NUM_AB_STATE"
}
